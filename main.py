...
class MetadataWorker(BaseWorker):
    '''
    Specialised worked for extracting metadata for given id
...
class AuthorWorker(BaseWorker):
    '''
    Specialised worked for extracting author's books for given id
    ...
    def _get_data(self) -> Optional[list[str]]:
        ...
        with access_data(self.browser.open(url, timeout=self.timeout)) as page:
    ...
    def _extract_authors_books(self, page: bytes) -> list[str]:
        ...
        for book in parsed_data.findall(xpath):
            ...
            if word_count / len(title_tokens) >= self.TITLE_THRESHOLD:
                result.append(found[1])
                no_to_find -= 1
